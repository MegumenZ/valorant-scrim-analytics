            ...
            leaderboard.map((item, idx) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-2.5">
                      <div className="shrink-0">
                        {idx === 0 ? (
                          <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-amber-400/20 text-amber-400 font-black text-xs">
                            1
                          </span>
                        ) : idx === 1 ? (
                          <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-slate-400/20 text-slate-300 font-black text-xs">
                            2
                          </span>
                        ) : idx === 2 ? (
                          <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-amber-700/20 text-amber-500 font-black text-xs">
                            3
                          </span>
                        ) : (
                          <span className="inline-flex items-center justify-center w-6 h-6 text-slate-400 font-bold text-xs">
                            {idx + 1}
                          </span>
                        )}
                      </div>
                      ...
                      <div>
                        <div className="text-xs font-bold text-slate-100 flex items-center gap-1.5">
                          ...
                          <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-semibold border ${roleColor.badge}`}>
                  ...
                  <div className="grid grid-cols-3 gap-1.5 pt-2 border-t border-[#242e40]/50 text-center">
                    <div className="p-1.5 rounded-lg bg-[#0e131b] border border-[#242e40]/70">
                      <div className="text-[9px] text-slate-400">Avg ACS</div>
                      <div className="text-xs font-black text-sky-400 tabular-nums">
                        {item.avgAcs}
                      </div>
                    </div>
                    <div className="p-1.5 rounded-lg bg-[#0e131b] border border-[#242e40]/70">
                      <div className="text-[9px] text-slate-400">K/D Ratio</div>
                      <div className={`text-xs font-black tabular-nums ${
                        item.kdRatio >= 1.2 ? "text-emerald-400" : item.kdRatio >= 1.0 ? "text-slate-200" : "text-rose-400"
                      }`}>
                        {item.kdRatio.toFixed(2)}
                      </div>
                    </div>
                    <div className="p-1.5 rounded-lg bg-[#0e131b] border border-[#242e40]/70">
                      <div className="text-[9px] text-slate-400">Headshot</div>
